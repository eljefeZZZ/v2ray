...
author=eljefeZZZ
# github=https://github.com/eljefeZZZ/v2ray.git
...
# 添加 ca-certificates 依赖以确保 HTTPS 验证正常
is_pkg="wget unzip ca-certificates"
...
# wget wrapper [安全修复: 注入防护 & SSL验证]
_wget() {
    # 修复：变量加引号防止注入
    [[ $proxy ]] && export https_proxy="$proxy"
    # 修复：移除 --no-check-certificate，启用 SSL 验证
    wget $*
}
...
main() {
    ...
    clear
    echo
    echo "........... $is_core_name script by $author (Secured Version) .........."
    echo
    ...
    # create tmpdir (already created securely)
    ...
    # add alias - 使用 grep 检查是否已存在，防止重复添加
